...
namespace LanShanPRMS.Areas.TeacherSys.Controllers
{
    ...
    public class HomeController : BaseTeacherController
    {
        ...
        {
            var tlist = _treatise.GetTeacherTreatises(ConfigHelper.TeacherID);
            ...
            var notice = _notice.GetTeacherNoticeCount(ConfigHelper.TeacherID);
            var list = _notice.GetListByType(0).Take(5).ToList();
            var model = new TreatiseTeacherViewModel(trea, ConfigHelper.TeacherID) { UnCount = notice, Notices = list };
            return View(model);
        }
